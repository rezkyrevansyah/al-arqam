...
export function Donation() {
  ...
  if (!data || !data.donation) return null;

  const bankAccountNumber = data.donation.bankAccountNumber?.trim() || '';
  const bankAccountName = data.donation.bankAccountName?.trim() || '';
  const bankName = data.donation.bankName?.trim() || 'Bank Muamalat';
  const donationCollected = data.donation.donationCollected || 0;
  const donationTarget = data.donation.donationTarget || 0;
  const rawQrisUrl = data.donation.qrisImageUrl?.trim() || '';
  const qrisImageUrl = formatImageUrl(rawQrisUrl);

  if (!bankAccountNumber && !rawQrisUrl) return null;
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.6 }}
                className={`${qrisImageUrl ? 'lg:col-span-3' : 'lg:col-span-5 max-w-3xl mx-auto'} bg-white border border-[hsl(var(--border))]/60 rounded-3xl p-6 md:p-8 shadow-xl shadow-black/5 relative overflow-hidden flex flex-col`}
              >
                ...
                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div>
                    ...
                    <div className="bg-[hsl(var(--muted))]/50 rounded-2xl p-4 flex items-center justify-between">
                      <div>
                  ...
                  {/* Progress - Only shown when donation target is set (> 0) */}
                  {donationTarget > 0 && (
                    <div className="mt-6 pt-6 border-t border-[hsl(var(--border))]/50">
                  ...
                  {/* Official Trust Note */}
                  <div className="mt-6 p-4 rounded-2xl bg-[hsl(var(--muted))]/40 border border-[hsl(var(--border))]/60 flex items-start gap-3">
                    <ShieldCheck className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-xs font-semibold text-[hsl(var(--foreground))]">
                        Rekening Resmi DKM Masjid Jami' Al-Arqam
                      </p>
                      <p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5 leading-relaxed">
                        Salin nomor rekening untuk transfer melalui ATM, Mobile Banking, atau Internet Banking. Seluruh dana yang terkumpul disalurkan secara amanah untuk operasional dan program dakwah masjid.
                      </p>
                    </div>
                  </div>
                </div>
